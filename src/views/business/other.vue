...
import {vipList, changeVipLevel, addBusinessBasic, otherList} from '@/api/member'
...
export default {
  ...
  methods: {
    ...
    paginationEvent(e){
      this.$router.push({path:'/business/other',query:{page:e.page, limit:e.limit}})
      this.getList()
    },
    getList() {
      this.listLoading = true
      console.log(this.listQuery)
      otherList(this.listQuery).then(res => {
        if(res.code == 200){
