...
import {vipList, changeVipLevel, addBusinessBasic, recruiterList} from '@/api/member'
...
export default {
  ...
  methods: {
    ...
    handleCreateJobs(row) {
      this.$router.push({ path: '/business/jobs/addJobs', query: { uid: row.user_id, cid: row.id, identity:2 }})
    },
    handleLookJobs(row){
      console.log(row)
    ...
    handleCreateEvents(row){
      this.$router.push({ path: '/business/events/addEvents', query: { uid: row.user_id,cid:row.id,identity:2 }})
    },
