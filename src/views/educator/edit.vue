<template>
  <div class="container">
    <el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="160px" class="ruleForm">
      ...
      <div class="account-info-container" v-if="educatorInfoData.vip_due_time">
        <div class="title">Account Info</div>
        <div class="account-item">
          <div class="account-item-label">Membership:</div>
          <div class="account-item-time">
            <span v-if="educatorInfoData.level == 1">Basic</span>
            <span v-if="educatorInfoData.level == 2">Pro</span>
            <span v-if="educatorInfoData.level == 3">Plus</span>
            <span v-if="educatorInfoData.level == 4">Lifetime</span>
          </div>
        </div>

        <div class="account-item">
          <div class="account-item-label">Expires on:</div>
          <template v-if="educatorInfoData.level == 4">
            <div class="account-item-time" >
              -
            </div>
          </template>
...
export default {
  ...
  methods: {
    ...
    getUserInfo(uid) {
      ...
      userInfo(data).then(res => {
        ...
        if (res.code === 200) {
          ...
          this.educatorInfoData = educatorInfo;
          this.vipDueTime =educatorInfo.vip_due_time
    ...
    vipConfirm(){
      ...
      let params = {
        user_id:this.educatorInfoData.user_id,
        vip_due_time:this.vipDueTime
      }

      let userId = this.$route.query.uid

      addEduBasic(params).then(res => {
        console.log(res)
        if (res.code === 200) {
